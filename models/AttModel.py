...
def sort_pack_padded_sequence(input, lengths):
	# input是 att_feat，
	# lengths是 att_masks.data.long().sum(1)， 因为使用的是独热编码，每个特征值值有一个1来表示，所有之可以表示为特征的长度

	# 由大到小进行排布，indices则是下表索引
	# descending=True : descending,倒序(由大到小)
	# descending=False: ascending,升序(由小到大)
	# indices是排序后的数组中在原数组下标的索引
	sorted_lengths, indices = torch.sort(lengths, descending=True)
	tmp = pack_padded_sequence(input[indices], sorted_lengths, batch_first=True)
	# tmp可作为LSTM的输入了
	# 如果batch_first为True，则input的形状应该为batch_size*longest_seq_length*size
	# 将一个填充后的变长序列压紧
...
class AttModel(CaptionModel):
	def __init__(self, opt):
		...
		self.att_embed = nn.Sequential(*(
									((nn.BatchNorm1d(self.att_feat_size),) if self.use_bn else ())+ # True则为(nn.BatchNorm1d(self.att_feat_size),) False则为()
									(nn.Linear(self.att_feat_size, self.rnn_size),
									nn.ReLU(),
									nn.Dropout(self.drop_prob_lm))+
									((nn.BatchNorm1d(self.rnn_size),) if self.use_bn==2 else ())))
		# use_bn：使用的BN(batch_normal)次数
	...
	def clip_att(self, att_feats, att_masks):
		...
		# mask是掩码数组，起到遮挡作用...
		if att_masks is not None:
			max_len = att_masks.data.long().sum(1).max()    # 找到一个在x方向上相加后，再找最大的数记为max_len
			att_feats = att_feats[:, :max_len].contiguous() # 经过contiguous后的才能进行view()进行reshape操作
			att_masks = att_masks[:, :max_len].contiguous()
		return att_feats, att_masks

	def _prepare_feature(self, fc_feats, att_feats, att_masks):
		att_feats, att_masks = self.clip_att(att_feats, att_masks)  # 将这俩弄到最长

		# embed fc and att feats
		fc_feats = self.fc_embed(fc_feats)
		att_feats = pack_wrapper(self.att_embed, att_feats, att_masks) # 得到context？？

		# Project the attentin feats first to reduce memory and computation comsumptions.
	...
	def _forward(self, fc_feats, att_feats, seq, att_masks=None):
		...
		# 相当于将c_feats进行view(batch_size, seq.size(1) - 1, self.vocab_size+1)
		# 然后再将中的所有的元素值其变成0
		outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)

		# Prepare the features
		p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)
		# pp_att_feats是给Attention Model使用的
		# pp_att_feats is used for attention, we cache it in advance to reduce computation cost

		# seq: [seq.length. batch_size] --> batch_size 指的是 sample numble?如下的for猜测
		for i in range(seq.size(1) - 1):
			# 进行采样
			# ss_prob：Schedule sampling probability
			if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
				# 等效于对batch_size进行uniform分布的操作
				sample_prob = fc_feats.new(batch_size).uniform_(0, 1)
				sample_mask = sample_prob < self.ss_prob
				# 若采样的mask矩阵每一个都是0，
				if sample_mask.sum() == 0:  #没有遮挡直接使用
					it = seq[:, i].clone()
				else:
					# 返回所有非0值的下标索引并展开成1维
					...
					it.index_copy_(0, sample_ind, torch.multinomial(prob_prev, 1).view(-1).index_select(0, sample_ind)) # 从it里重新选择表示遮挡作用?
					# 将torch.multinomial(prob_prev, 1).view(-1).index_select(0, sample_ind)的在第sample_ind处的行Copy到it里
					# sample_ind给出了需要选择的下标索引
